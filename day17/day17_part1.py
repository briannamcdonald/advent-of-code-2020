def create_neighbour_list(x, y, z):
    neighbour_list = []
    for x_val in range(x - 1, x + 2):
        for y_val in range(y - 1, y + 2):
            for z_val in range(z - 1, z + 2):
                if x_val == x and y_val == y and z_val == z:
                    continue
                neighbour_list.append((x_val, y_val, z_val))
...
def update_space(coord_dict):
    ...
    for coord in coord_dict:
        if coord_dict[coord] == "#":
            neighbour_list = create_neighbour_list(coord[0], coord[1], coord[2])
